...
# Phase 3: 全面知識庫擴充與防汙染架構 (Deduplication)

為了解決新法規納入、勞發署(WDA)全數 12 項資料，以及避免後續重複執行爬蟲造成 RAG 資料庫「汙染 (Pollution)」的問題，我們將實作防呆機制與附件下載。

## Proposed Changes

### 1. 新增三大核心法規與附表下載
在 `crawl-laws.ts` 的標的清單中加入以下三法：
- `N0090027`: 雇主聘僱外國人許可及管理辦法
- `L0050018`: 受聘僱外國人健康檢查管理辦法
- `N0090023`: 外國人受聘僱從事就服法第46條第1項第8至11款規定工作之轉換雇主或工作程序準則
- **[NEW] 處理附表 (Appendices)**: 當爬蟲在這些法規遇到附表連結時，將會自動透過 HTTP 抓取 Word/PDF/Excel 檔案，並統一存放於 `D:\AMAN\lawbridge\raguse\` 目錄中，並在 RAG Chunk 的 text 內保留對應的本機檔案路徑或檔名與連結，讓 AI 可以提供附件指引。

### 2. 升級 WDA 全面爬蟲
目前的 `crawl-wda-faq.ts` 僅抓取了第一頁的 QA，後續將大幅升級：
- 遍歷並涵蓋 `專業證照`、`訓練課程`、`實作認定`、`常見問答QA` 等 12 個子類別。
- 遇到下載資源，同樣將檔案儲存至 `raguse/` 目錄。

### 3. RAG 防重複汙染與即時更新機制 (Batch ID Pruning)
**問題痛點**：舊法規若被刪除，重新 Embedding 時舊 Chunk 依舊會殘留在資料庫，造成幻覺或矛盾。
**架構設計 (Idempotent + Prune)**：
- 修改 `chunk-laws.ts` 與上傳腳本，**每次執行上傳時，賦予一個統一的 `batchVersion` (例如 Timestamp)。**
- 上傳並覆蓋現有的 Chunk (Firestore 的 `set(..., {merge: true})` 會利用相同的 Chunk ID 自動更新內容，不會重複新增)。
- **清理舊資料 (Pruning)**：每個 `sourceId` 上傳完畢後，執行資料庫查詢：`Delete from kb_chunks where sourceId == X AND batchVersion != currentBatchVersion`。
- **結論**：這樣就能確保無論您未來重新執行爬蟲多少次，Firestore 永遠只保留「最新那一次掃到的乾淨資料」，完美解決重複與法規廢止的汙染問題。未來如果要做到「即時更新」，只要將此腳本放到排程伺服器 (如 GitHub Actions) 每天半夜定期執行即可。

## Verification Plan (Phase 3)
1. 檢查 `D:\AMAN\lawbridge\raguse` 資料夾是否成功下載各大法規的附件 (.pdf, .doc)。
2. 檢查 `_all_chunks.json` 是否包含上述 3 部新法規。
3. 執行兩次上傳腳本，前往 Firebase Console 驗證 Document 數量完全沒有增加（證實 Batch Pruning 與防汙染機制生效）。
